...
export async function getWeekSummary() {
  ...
  const result = await db
    .with(goalsCreateUptToWeek, goalsCompletedInWeek, goalsCompletedByWeekDay)
    .select({
      completed: sql`(SELECT COUNT(*) FROM ${goalsCompletedInWeek})`.mapWith(
        Number
      ),
      total:
        sql`(SELECT SUM(${goalsCreateUptToWeek.desiredWeeklyFrequency}) FROM ${goalsCreateUptToWeek})`.mapWith(
          Number
        ),
